...
x = np.array([blue[id].x , blue[id].y , blue[id].orientation, 0.0, 0.0])
traj = np.array(x)
...
while True:
	...
	print(u)
	print(blue[0])
	x = np.array([blue[id].x , blue[id].y , blue[id].orientation, u[0] , u[1] ])
	if math.sqrt((x[0] - goal[0]) ** 2 + (x[1] - goal[1]) ** 2) <=  1:
		print("Goal!!")
		i = i + 1
		#if i is len(path):
